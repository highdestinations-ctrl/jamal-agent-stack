...
export class MemoryService {
  ...
  async createWorkingMemory(memory: Omit<WorkingMemory, "createdAt" | "updatedAt">): Promise<WorkingMemory> {
    const query = `
  ...
  async getWorkingMemory(taskId: string): Promise<WorkingMemory | null> {
  ...
  async updateWorkingMemory(taskId: string, updates: Partial<WorkingMemory>): Promise<WorkingMemory> {
    const query = `
  ...
  async deleteWorkingMemory(taskId: string): Promise<void> {
  ...
  async createSessionMemory(sessionId: string, messages: SessionMemory["messages"]): Promise<SessionMemory> {
    const query = `
  ...
  async appendSessionMessage(sessionId: string, message: SessionMemory["messages"][0]): Promise<SessionMemory> {
  ...
  async summarizeSession(sessionId: string, summary: string): Promise<SessionMemory> {
    const query = `
  ...
  async retrieveLongTermMemory(options: MemoryQueryOptions): Promise<LongTermMemory[]> {
  ...
  async semanticSearch(sessionId: string, embedding: number[], limit: number = 5): Promise<LongTermMemory[]> {
    try {
  ...
  async createOrUpdateSupervisorState(state: SupervisorState): Promise<SupervisorState> {
    const query = `
  ...
  async getSupervisorState(sessionId: string): Promise<SupervisorState | null> {
  ...
  private rowToWorkingMemory(row: Record<string, any>): WorkingMemory {
    return {
      taskId: row.task_id,
      sessionId: row.session_id,
      context: row.context,
  ...
  private rowToSessionMemory(row: Record<string, any>): SessionMemory {
    return {
      sessionId: row.session_id,
  ...
  private rowToSupervisorState(row: Record<string, any>): SupervisorState {
    return {
      sessionId: row.session_id,
...
export async function createMemoryService(pgConfig: any, qdrantConfig: any): Promise<MemoryService> {
